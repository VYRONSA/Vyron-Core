...
| **8 (REQUIRED — was mislabelled optional)** | `sql/008-demo-tier-timestamp.sql` | Adds `demo_started_at` on `companies` for the 30-day unlimited **Demo** tier. The client directory query selects this column, so a project without it fails with `column companies.demo_started_at does not exist`. **Already folded into `sql/069-customer-user-management.sql`** — run 069 and you do not need this file separately. |
| **69 (required for user management)** | `sql/069-customer-user-management.sql` | Profile / lifecycle / module-access columns on `company_users` for Settings → Users & Access and Platform Console → Customer → Users, plus the `demo_started_at` column from 008. Backwards compatible; no data is deleted. |
