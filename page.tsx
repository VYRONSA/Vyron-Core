    ...
    <div className="space-y-6">
      <Panel dark>
        <div className="relative z-10">
          ...
          <h2 className="mt-3 text-4xl font-black tracking-tight text-white">{title}</h2>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {cards.map((card) => (
          <button
            ...
            className="group rounded-[28px] border border-white/80 bg-white/95 p-6 text-left shadow-[0_18px_55px_rgba(15,23,42,0.12)] transition hover:-translate-y-1 hover:border-cyan-200 hover:shadow-[0_28px_80px_rgba(37,99,235,0.18)]"
          >
            <div className="flex items-start justify-between gap-4">
              <div className="rounded-2xl bg-[#06101f] p-3 text-cyan-300 shadow-lg shadow-cyan-950/15">
...
function IntegrationsHubScreen({ setActive }: { setActive: (value: string) => void }) {
  return (
    <ActionHubScreen
      title="Integrations"
      subtitle="Connect VYRON CORE to payroll, finance and reporting workflows. Start with clean exports, then move to live integrations."
      setActive={setActive}
      cards={[
        { title: "Payroll Export", description: "Prepare payroll files for Sage, SimplePay or CSV handoff.", target: "Payroll Export Centre", badge: "Ready" },
        { title: "Reports", description: "Open reporting outputs that can support finance and management reviews.", target: "Reports Centre" },
        { title: "Payroll Prep", description: "Check payroll readiness before exporting to payroll systems.", target: "Payroll Prep" },
        { title: "Audit History", description: "Review historical approvals, changes and exported records.", target: "History Reports" },
        { title: "Settings", description: "Manage company users, permissions and system setup.", target: "Settings / Roles" },
        { title: "Future APIs", description: "Xero, Sage, SimplePay and PayProp integrations can be activated after core validation.", target: "Reports Centre", badge: "Future" },
      ]}
    />
  );
}

function PayrollHubScreen({
  payrollBatches,
  payrollHours,
  rosterShifts,
  clockEvents,
  ...
  return (
    <div className="space-y-6">
      <ActionHubScreen
        title="Payroll"
        subtitle="Payroll readiness, clocking exceptions, approved hours and export preparation in one workflow."
        setActive={setActive}
        cards={[
          { title: "Payroll Prep", description: "Review readiness, blockers, approved hours and missing clocking issues.", target: "Payroll Prep", badge: "Main" },
          { title: "Payroll Checks", description: "Open clock-to-payroll checks and manager review status.", target: "Payroll Clock Engine" },
          { title: "Exceptions", description: "Resolve open clocking and time exceptions before payroll export.", target: "Exceptions" },
          { title: "Export Payroll", description: "Prepare approved payroll data for export.", target: "Payroll Export Centre" },
          { title: "Reports", description: "Open payroll and workforce reporting summaries.", target: "Reports Centre" },
          { title: "Employees", description: "Check employee records connected to payroll readiness.", target: "Employees" },
        ]}
      />
      <PayrollPrepScreen
...
function SmartAlertsHubScreen({
  ...
  const openExceptions = exceptions.filter(exceptionIsOpen).length;
  const openHrCases = hrCases.filter(hrCaseIsOpen).length;
  const payrollReviews = payrollHours.filter(rowHasPayrollProblem).length;

  return (
    <ActionHubScreen
      title="Smart Alerts"
      subtitle="Operational alerts that point managers to the next action: clocking, HR, payroll and workforce risk."
      setActive={setActive}
      cards={[
        { title: `${openExceptions} Open Exceptions`, description: "Resolve clocking and operational exceptions.", target: "Exceptions", badge: "Action" },
        { title: `${openHrCases} HR Cases`, description: "Review HR cases that may block payroll or compliance.", target: "HR Cases" },
        { title: `${payrollReviews} Payroll Reviews`, description: "Check payroll rows needing manager review.", target: "Payroll Prep" },
        { title: "Clocking Review", description: "Review missing clock-ins, clock-outs and time mismatches.", target: "Clocking Review" },
        { title: "Notifications", description: "Open employee and manager notifications.", target: "Employee Notifications" },
        { title: "Reports", description: "Review risk and performance reports.", target: "Reports Centre" },
      ]}
    />
  );
}

function AIHubScreen({ setActive }: { setActive: (value: string) => void }) {
  return (
    <ActionHubScreen
      title="AI Assistant"
      subtitle="AI guidance hub for HR, clocking, payroll readiness and operational decisions. Live AI actions can be connected after workflow validation."
      setActive={setActive}
      cards={[
        { title: "HR Guidance", description: "Open HR cases and warnings that need review.", target: "HR Cases" },
        { title: "Payroll Guidance", description: "Open payroll readiness and blocker checks.", target: "Payroll Prep" },
        { title: "Clocking Guidance", description: "Open clocking review and time exceptions.", target: "Clocking Review" },
        { title: "Insights", description: "Review workforce trends and risk indicators.", target: "Workforce Intelligence" },
        { title: "Smart Alerts", description: "Open operational alerts and next actions.", target: "Smart Detection" },
        { title: "Reports", description: "Open reporting centre for exportable evidence.", target: "Reports Centre" },
      ]}
    />
  );
}
...
export default function Page() {
  ...
  const [activeSidebarGroup, setActiveSidebarGroup] = useState("Main");

  function setActive(next: string) {
    const nextGroup =
      navGroups.find((group) =>
        group.items.some((item) => item === next || resolveNavigationTarget(item) === next)
  ...
  function renderSection() {
    ...
    if (active === "Payroll") return <PayrollHubScreen payrollBatches={payrollBatches} payrollHours={payrollHours} rosterShifts={rosterShifts} clockEvents={clockEvents} exceptions={exceptions} hrCases={hrCases} employees={employees} companyId={currentCompanyId} onRefresh={refreshData} setActive={setActive} />;
    ...
    if (active === "Integrations") return <IntegrationsHubScreen setActive={setActive} />;
    if (active === "AI Assistant") return <AIHubScreen setActive={setActive} />;
    if (active === "Smart Alerts") return <SmartAlertsHubScreen exceptions={exceptions} hrCases={hrCases} payrollHours={payrollHours} setActive={setActive} />;
