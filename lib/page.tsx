    ...
    <Panel>
      <div className="py-12 text-center">
        ...
        <p className="mx-auto mt-3 max-w-2xl text-sm leading-6 text-slate-500">
          This workspace is being connected to a live workflow. Use the button below to return to the Dashboard while this module is wired into the main system.
        </p>
...
export default function Page() {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      const cleanEmail = String(authUserEmail || "").trim().toLowerCase();
      ...
      if (currentUserError?.message || !currentUserData?.user?.id) {
        accessErrorMessage = currentUserError?.message || "No logged-in Supabase Auth user was found.";
      } else {
        const activeAuthUserId = currentUserData.user.id;

        const { data: companyUserRows, error: companyUserError } = await supabase
          .from("company_users")
          .select("company_id, user_id, user_email, role, status")
          .eq("user_id", activeAuthUserId)
          .eq("status", "active")
          .limit(1);

        if (companyUserError?.message) {
          accessErrorMessage = companyUserError.message;
        }

        const companyUser =
          Array.isArray(companyUserRows) && companyUserRows.length > 0
            ? companyUserRows[0]
            : null;
        ...
        if (companyUser?.company_id) {
          ...
          if (!directCompanyError && directCompany?.id) {
            access = {
              company_id: directCompany.id,
              company_name: directCompany.name || "Active company",
              role: companyUser.role || "admin",
              user_status: companyUser.status || "active",
              company_status: directCompany.status || "active",
              subscription_status: directCompany.subscription_status || "active",
              subscription_locked: false,
            };
          } else if (directCompanyError?.message) {
            accessErrorMessage = directCompanyError.message;
          }
        }
      }

      if (!access?.company_id) {
        setError(`Company access issue: ${accessErrorMessage || `No active VYRON CORE company access found for ${cleanEmail}. Ask the system owner to add this user under Company Users.`}`);
        setCurrentCompanyId("");
        setCurrentUserRole("");
      ...
      if (String(access.company_status || "active").toLowerCase() !== "active") {
        setError("This company is not active. Access is locked until the company is activated.");
        setCurrentCompanyId(access.company_id);
        setCurrentUserRole(access.role || "manager");
        setCurrentCompanyName(access.company_name || "Inactive company");
        setLoading(false);
        return;
      }

      if (!["active", "trialing", "demo"].includes(String(access.subscription_status || "active").toLowerCase())) {
        setError("This company subscription is not active. Please contact VYRON billing to unlock access.");
        setCurrentCompanyId(access.company_id);
        setCurrentUserRole(access.role || "manager");
        setCurrentCompanyName(access.company_name || "Subscription inactive");
        setLoading(false);
        return;
      }

      const activeCompanyId = access.company_id;
      const activeRole = access.role || "manager";
      const activeCompanyName = access.company_name || "Active company";

      setCurrentCompanyId(activeCompanyId);
      setCurrentUserRole(activeRole);
      setCurrentCompanyName(activeCompanyName);

      const [storesRes, employeesRes, exceptionsRes, hrCasesRes, rosterRes, clockRes, payrollRes, payrollHoursRes, payrollClockChecksRes, rolesRes, hrWarningsRes, hrDocumentsRes, employeeDocumentsRes, hrNotesRes, leaveRequestsRes] = await Promise.all([
        ...
        supabase.from("hr_cases").select("id,employee_id,linked_exception_id,case_type,title,description,validity_status,status,employee_response_required,employee_response,manager_feedback").eq("company_id", activeCompanyId).order("created_at", { ascending: false }),
        ...
        supabase.from("employee_documents").select("*").eq("company_id", activeCompanyId).order("created_at", { ascending: false }),
        supabase.from("hr_notes").select("*").neq("status", "deleted").order("created_at", { ascending: false }),
        supabase.from("leave_requests").select("*").order("created_at", { ascending: false }),
      ]);

      const firstError = storesRes.error || employeesRes.error || exceptionsRes.error || hrCasesRes.error || rosterRes.error || clockRes.error || payrollRes.error || payrollHoursRes.error || payrollClockChecksRes.error || rolesRes.error || hrWarningsRes.error || hrDocumentsRes.error || employeeDocumentsRes.error || hrNotesRes.error || leaveRequestsRes.error;
      ...
      if (firstError) {
        ...
        setEmployeeDocuments((employeeDocumentsRes.data || []) as EmployeeDocumentRow[]);
  ...
  function renderSection() {
    ...
    if (active === "Workforce Intelligence") return <ConnectedInsightsScreen stores={stores} employees={employees} exceptions={exceptions} hrCases={hrCases} payrollHours={payrollHours} payrollClockChecks={payrollClockChecks} setActive={setActive} />;
   ...
   if (active === "Leave Approvals") return <LeaveApprovalsScreen leaveRequests={leaveRequests} employees={employees} onRefresh={refreshData} setHrCaseModalOpen={() => {}} />;
