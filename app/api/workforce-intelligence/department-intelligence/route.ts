...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ ok: true, departments: intelligence.departments.slice(0, limit) });
  } catch (error: unknown) {
    const message = error instanceof Error ? error.message : "Could not compute department intelligence.";
