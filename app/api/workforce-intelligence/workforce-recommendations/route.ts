...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ ok: true, recommendations: intelligence.recommendations.slice(0, limit) });
  } catch (error: unknown) {
    const message = error instanceof Error ? error.message : "Could not compute workforce recommendations.";
