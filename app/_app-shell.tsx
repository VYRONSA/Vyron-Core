...
import {
  ...
  formatDemoRequestSubmittedAt,
  isDeletableDemoRequest,
  loadDemoRequestsWithSupabase,
  persistDemoRequestDeletes,
  persistDemoRequestStatus,
  VYRON_DEMO_REQUESTS_STORAGE_KEY,
  VYRON_MASTER_INBOX_CHANGED_EVENT,
  type DemoRequestRow,
...
import {
  attachCompanyUsersToDirectoryRows,
  COMPANY_USERS_DIRECTORY_EMBED,
  getCompanyAccess,
...
import VyronDevMasterDashboard from "../components/vyron-dev/VyronDevMasterDashboard";
import VyronDevClientDirectoryScreen from "../components/vyron-dev/VyronDevClientDirectoryScreen";
import VyronDevActiveClientPanel from "../components/vyron-dev/VyronDevActiveClientPanel";
import VyronDevBillingPanel from "../components/vyron-dev/VyronDevBillingPanel";
import VyronDevIntegrationsPanel from "../components/vyron-dev/VyronDevIntegrationsPanel";
import VyronDevProductWorkspaceScreen from "../components/vyron-dev/VyronDevProductWorkspaceScreen";
import VyronDevProvisioningPanel from "../components/vyron-dev/VyronDevProvisioningPanel";
import VyronDevWorkspaceProvisioningPanel from "../components/vyron-dev/VyronDevWorkspaceProvisioningPanel";
import VyronDevPackageEnginePanel from "../components/vyron-dev/VyronDevPackageEnginePanel";
import VyronDevDeploymentCentrePanel from "../components/vyron-dev/VyronDevDeploymentCentrePanel";
import VyronDevSupportSessionBanner from "../components/vyron-dev/VyronDevSupportSessionBanner";
import FieldOperationsDashboard from "../components/field-ops/FieldOperationsDashboard";
import JobVisitsPanel from "../components/field-ops/JobVisitsPanel";
import JobTimelineEngine from "../components/field-ops/JobTimelineEngine";
import EmployeeFieldWorkflow from "../components/field-ops/EmployeeFieldWorkflow";
import MobileWorkforceHub from "@/components/mobile-workforce/MobileWorkforceHub";
import WorkforceJourneyPanel from "../components/field-ops/WorkforceJourneyPanel";
import TravelIntelligencePanel from "../components/field-ops/TravelIntelligencePanel";
import RouteHistoryPanel from "../components/field-ops/RouteHistoryPanel";
import VehicleAssetIntelligencePanel from "../components/field-ops/VehicleAssetIntelligencePanel";
import ClientProfitabilityIntelligencePanel from "../components/field-ops/ClientProfitabilityIntelligencePanel";
...
import WorkforceAutomationCentrePanel from "../components/field-ops/WorkforceAutomationCentrePanel";
import WhatsAppCommandCentrePanel from "../components/field-ops/WhatsAppCommandCentrePanel";
...
async function queryCompaniesDirectory(options?: {
  ...
  if (options?.companyId) {
    bareQuery = bareQuery.eq("id", options.companyId);
  } else {
    bareQuery = bareQuery.order("created_at", { ascending: false });
  }
  ...
  if (
    bareResult.error &&
    withProfile &&
    isMissingCompaniesProfileColumnError(bareResult.error.message)
  ) {
    return queryCompaniesDirectory({ ...options, withProfile: false });
  }

  if (bareResult.error) {
    if (isSupabaseRelationshipError(bareResult.error)) {
      return { rows: [], single: null, error: null };
    }
    return { rows: [], single: null, error: bareResult.error.message };
  }

  const bareData = bareResult.data as unknown;
  const rawRows = options?.companyId
    ? bareData
      ? [bareData as Record<string, unknown>]
      : []
    : ((bareData as Record<string, unknown>[] | null) || []);

  const merged = await attachCompanyUsersToDirectoryRows(supabase, rawRows);
  return {
    rows: merged,
    single: options?.companyId ? merged[0] ?? null : null,
    error: null,
  };
}
...
/** Active employee cap per tier (null = unlimited). Aligns with CORE package employee limits. */
...
function formatSubscriptionTierFeeLabel(tier: (typeof CLIENT_SUBSCRIPTION_TIERS)[number]): string {
  if (tier === "Enterprise") return "Contact Sales / Custom";
  const fee = VYRON_SUBSCRIPTION_TIER_MONTHLY_FEES[tier];
  if (fee === 0) return "Demo / Free";
  return `R ${fee.toLocaleString("en-ZA")}/mo`;
}
...
function readClientDirectoryFromStorage(): MasterClientDirectoryEntry[] {
  ...
  try {
    ...
    return parsed.filter((entry) => !isExcludedFromMasterClientDirectory(String(entry.id || "")));
  } catch {
    return [];
  }
}
...
function NavIcon({ item }: { item: string }) {
  ...
  if (item.includes("Field Operations") || item.includes("Job Visits")) return <MapPin className="h-5 w-5" />;
  if (item.includes("Job Timeline")) return <Route className="h-5 w-5" />;
  if (item.includes("Field Mobile")) return <Smartphone className="h-5 w-5" />;
  if (item.includes("Workforce Journey")) return <Navigation className="h-5 w-5" />;
  if (item.includes("Travel Intelligence")) return <Route className="h-5 w-5" />;
  if (item.includes("Route History")) return <CalendarRange className="h-5 w-5" />;
  if (item.includes("Vehicle Intelligence")) return <Truck className="h-5 w-5" />;
  if (item.includes("Workforce Cost")) return <WalletCards className="h-5 w-5" />;
  if (item.includes("Profitability Intelligence")) return <TrendingUp className="h-5 w-5" />;
  if (item.includes("Client Portal")) return <Users className="h-5 w-5" />;
  if (item.includes("Training Centre")) return <BookOpen className="h-5 w-5" />;
  if (item.includes("Pilot Readiness")) return <Rocket className="h-5 w-5" />;
  if (item.includes("Workforce Risk")) return <ShieldAlert className="h-5 w-5" />;
  if (item.includes("AI Command")) return <Sparkles className="h-5 w-5" />;
  if (item.includes("WhatsApp Command")) return <MessageSquare className="h-5 w-5" />;
  if (item.includes("Digital Twin")) return <Activity className="h-5 w-5" />;
  if (item.includes("Payroll Intelligence") || item.includes("Payroll Readiness") || item.includes("Payroll Leakage") || item.includes("Payroll Forecast") || item.includes("Payroll Exceptions")) return <WalletCards className="h-5 w-5" />;
  if (item.includes("Workforce Lifecycle")) return <Users className="h-5 w-5" />;
  if (item.includes("Recruitment") || item === "Vacancies" || item === "Applicants" || item === "Interviews" || item.includes("Skills Intelligence") || item.includes("Succession")) return <UserPlus className="h-5 w-5" />;
  if (item.includes("Command Centre") || item.includes("Operating Dashboard") || item.includes("Automation Library") || item.includes("Health Score")) return <HeartPulse className="h-5 w-5" />;
  ...
  if (item.includes("Login As Client")) return <LogIn className="h-5 w-5" />;
  if (item.includes("Billing")) return <WalletCards className="h-5 w-5" />;
  if (item.includes("Integrations Status") || item.includes("VYRON DEV")) return <Zap className="h-5 w-5" />;
...
function isOnboardedTenantCompanySetup(
  ...
  return resolveTenantPermissionLayer(userRole, userEmail) === "owner";
}
...
function buildSidebarNavGroups(
  ...
  if (coreSupportMode) {
    const filteredMain = filterSidebarNavGroupsByRbac(
      navGroups,
      "owner",
      userEmail,
      resolveNavigationTarget
    );
    const workspaceGroup = hasCompanyAccess
      ? buildTenantWorkspaceNavGroup("owner", TENANT_SEND_FEEDBACK_ROUTE)
      : null;
    return workspaceGroup ? [workspaceGroup, ...filteredMain] : filteredMain;
  }
  ...
  const workspaceGroup =
    hasCompanyAccess && !isVyronMasterOperator(userRole, userEmail)
      ? buildTenantWorkspaceNavGroup(layer, TENANT_SEND_FEEDBACK_ROUTE)
      : null;
...
function Sidebar({
  ...
  const visibleNavGroups = buildSidebarNavGroups(
    userRole,
    userEmail,
    hasCompanyAccess,
    coreSupportMode
            ...
            >
              {upgradeOptions.map((tier) => (
                <option key={tier} value={tier}>
                  {tier} — {formatSubscriptionTierFeeLabel(tier)} — up to{" "}
...
function AddEmployeeModal({
  ...
  async function saveEmployee() {
    ...
    if (newEmployee?.id) {
      const seedResult = await seedEmployeeLeaveBalances(supabase, {
        companyId,
        employeeId: newEmployee.id,
        employeeName: `${newEmployee.first_name || firstName} ${newEmployee.last_name || lastName}`.trim(),
      });
      if (!seedResult.ok) {
        setError(`Employee created but leave balances could not be seeded: ${seedResult.error}`);
        setSaving(false);
        return;
      }
    }
...
function LeaveApprovalsScreen({
  ...
  async function cancelLeaveRequest(request: LeaveRequestRow) {
    ...
    const scopedCompanyId = request.company_id || companyId;
    const { error: updateError } = await supabase
      .from("leave_requests")
      .update({ status: "cancelled", manager_feedback: "Cancelled by manager." })
      .eq("id", request.id)
    ...
    if (updateError) {
      ...
      setError("Leave request cancelled.");
      onRefresh();
    }
    setSavingId(null);
  }
  ...
  async function updateLeaveStatus(request: LeaveRequestRow, status: "approved" | "declined" | "amended") {
    ...
    const { error: updateError } = scopedCompanyId
      ? await updateQuery.eq("company_id", scopedCompanyId)
      : await updateQuery;
                ...
                <article key={request.id} className="rounded-[2rem] border border-white/80 bg-white/90 p-5 shadow-[0_16px_45px_rgba(15,23,42,0.10)] backdrop-blur-xl">
                  ...
                  {request.status === "pending" && (
                    <div className="mt-4 flex flex-wrap gap-2">
                      ...
                      <button
                        onClick={() => cancelLeaveRequest(request)}
                        disabled={savingId === request.id}
                        className="rounded-2xl bg-slate-600 px-4 py-3 text-sm font-bold text-white disabled:opacity-60"
                      >
                        Cancel
                      </button>
                    </div>
...
function EmployeesScreen({
  ...
  async function deleteEmployee(employee: EmployeeRow) {
    ...
    const { error: deleteError } = await supabase
      .from("employees")
      .update({ active: false, record_status: "deleted" })
      .eq("id", employee.id)
      .eq("company_id", companyId);
    ...
    setSelectedEmployeeId(null);
    setEmployeeSaveMessage("Employee marked deleted (soft delete — record retained for audit).");
    setSavingEmployee(false);
    onRefresh();
  }
            ...
            <div className="mt-6 rounded-[2rem] border border-rose-100 bg-rose-50/80 p-5">
              ...
              <div className="mt-4 flex flex-wrap gap-3">
                {setActive && (
                  <button
                    type="button"
                    onClick={() => setActive("Workforce Movement")}
                    disabled={savingEmployee}
                    className="inline-flex items-center gap-2 rounded-2xl bg-cyan-600 px-5 py-3 text-sm font-black text-white disabled:opacity-60"
                  >
...
function ClientDirectoryDetailModal({
  ...
  useEffect(() => {
    ...
    async function loadFresh() {
      ...
      const directory = await queryCompaniesDirectory({ companyId: entry.id });

      if (cancelled) return;

      if (directory.error) {
        setLoadError(directory.error);
        setLoading(false);
        return;
      }

      const data = directory.single;
...
function DemoRequestsPanel() {
  ...
  useEffect(() => {
    ...
    async function reloadRows() {
      setLoading(true);
      try {
        const next = await loadDemoRequestsWithSupabase();
    ...
    function onStorage(event: StorageEvent) {
      if (event.key === VYRON_DEMO_REQUESTS_STORAGE_KEY) void reloadRows();
    }
  ...
  async function updateStatus(id: string, status: DemoRequestRow["status"]) {
    const next = await persistDemoRequestStatus(id, status);
    setRows(next);
  }

  async function deleteDemoRequests(ids: string[], successMessage: string) {
    const { nextRows } = await persistDemoRequestDeletes(ids);
    setRows(nextRows);
    setToast(successMessage);
  }

  async function handleDeleteRow(row: DemoRequestRow) {
    ...
    setBusyId(row.id);
    try {
      await deleteDemoRequests(
        [row.id],
        `Deleted demo request for ${row.name}.`
      );
    } finally {
      setBusyId(null);
    }
  ...
  async function handleDeleteAllOld() {
    ...
    try {
      await deleteDemoRequests(
        ...
        `Deleted ${deletable.length} old demo request${deletable.length === 1 ? "" : "s"}.`
      );
    } finally {
      setBatchBusy(false);
    }
  ...
  return (
    <Panel>
      {toast && (
        <div
          role="status"
          className="fixed bottom-6 left-1/2 z-[90] -translate-x-1/2 rounded-2xl border border-emerald-200 bg-emerald-50 px-5 py-3 text-sm font-semibold text-emerald-900 shadow-[0_16px_48px_rgba(16,185,129,0.25)]"
        >
          {toast}
        </div>
      )}
      ...
      {rows.length === 0 && !loading ? (
        <div className="mt-8 flex flex-col items-center justify-center rounded-[2rem] border border-dashed border-slate-200 bg-slate-50/80 px-8 py-16 text-center">
          <div className="rounded-2xl bg-white p-4 text-slate-400 shadow-sm">
            <Inbox className="h-8 w-8" />
          </div>
          <h3 className="mt-5 text-xl font-black text-slate-900">No demo requests</h3>
          <p className="mt-2 max-w-md text-sm leading-6 text-slate-500">
            Inbound demo interest from the website and sales channels will appear here. Deleted leads stay
...
function MasterExecutiveCommandCentre({
  ...
  clientDirectory: MasterClientDirectoryEntry[];
  platformState?: import("../lib/vyron-dev-platform").VyronDevPlatformState;
  onRefresh: () => void;
  onLogout: () => void | Promise<void>;
  setActive: (value: string) => void;
}) {
      ...
      <div className="relative z-10 space-y-6">
        ...
        <section className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
          ...
          >
            ...
            <div className="mt-2 text-2xl font-black text-[#06101f]">Manage clients &amp; products</div>
          </button>
          <button
            type="button"
          ...
          <button
            ...
            className="rounded-[2rem] border border-white/80 bg-white/95 p-6 text-left transition hover:-translate-y-1"
          >
            <div className="text-sm font-bold text-slate-500">Login As Client</div>
            <div className="mt-2 text-2xl font-black text-[#06101f]">Active client context</div>
          </button>
          <button
            type="button"
            onClick={() => setActive("Billing")}
            className="rounded-[2rem] border border-emerald-200 bg-emerald-50/90 p-6 text-left transition hover:-translate-y-1"
          >
            <div className="text-sm font-bold text-slate-500">Billing</div>
            <div className="mt-2 text-2xl font-black text-[#06101f]">Revenue foundation</div>
          </button>
        </section>
      </div>
    </section>
  ...
  return (
    <div className="space-y-8">
      <Panel dark>
        ...
        <h2 className="mt-3 text-3xl font-black tracking-tight">Mobile workforce platform</h2>
        <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-300">
          Production hub for technicians, guards, drivers, farm & maintenance teams — offline sync, GPS
          evidence, tasks, documents, notifications and incidents.
        </p>
      </Panel>
      {companyId ? (
        <MobileWorkforceHub
...
function ClientSetupScreen({
  ...
  async function handleProvision() {
    ...
    const passwordError = validateClientLoginPassword(adminPassword, confirmAdminPassword);
    if (passwordError) {
    ...
    try {
      const { company, error: provisionError } = await provisionClientCompany(
        supabase,
        legalName.trim(),
      ...
      let loginUserWarning: string | null = null;

      const { data: sessionData } = await supabase.auth.getSession();
      const accessToken = sessionData.session?.access_token;
      if (!accessToken) {
        loginUserWarning = "Client workspace saved, but operator session token was missing for login user creation.";
      } else {
        const loginResult = await requestCreateClientLoginUser(
          {
        ...
        if (loginResult.ok) {
          ...
          loginUserWarning = loginResult.message;
          resolvedInvite = {
            email: trimmedAdminEmail,
      ...
      setInviteStatus(resolvedInvite?.status || null);
      ...
      const adminNote = resolvedInvite
        ? `Administrator login: ${resolvedInvite.email} — ${resolvedInvite.status}.`
      ...
      setSavedMessage(
        `Client workspace provisioned for ${company.name || legalName.trim()} (${subscriptionTier} tier). ${adminNote}${
          loginUserWarning ? ` ${loginUserWarning}` : ""
        }${profileNote}`
      );
      setAdminPassword("");
...
function PayrollHardeningScreen({ companyId }: { companyId: string }) {
  return (
    <div className="space-y-8">
      <Panel dark>
      ...
      <PayrollHardeningCentre companyId={companyId} />
    </div>
  );
}
  ...
  return (
    <div className="min-h-screen bg-[#07101f] p-4 text-white md:p-8">
      ...
      <div className="mt-6 grid gap-5 lg:grid-cols-3">
        ...
        <ActionHubCard title="Leave Approvals" value={pendingLeave} subtitle="Open pending leave decisions." target="Leave Approvals" setActive={setActive} />
      </div>
...
function AutomationWorkflowHubScreen({
  setActive,
  companyId,
  userEmail,
}: {
  ...
  return (
    <div className="space-y-8">
      {companyId ? (
        <WorkforceAutomationCentrePanel companyId={companyId} userEmail={userEmail} />
      ) : (
        <Panel>
          <p className="text-sm font-semibold text-slate-600">
            Sign in to a company workspace to use the Workforce Automation Engine.
          </p>
        </Panel>
      )}
      <Panel dark>
        <div className="text-xs font-bold uppercase tracking-[0.4em] text-cyan-300">Workflow shortcuts</div>
        <h2 className="mt-3 text-3xl font-black tracking-tight">Automation action hub</h2>
        <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-300">
          Open live workflows for exceptions, payroll, alerts, and notifications.
        </p>
      </Panel>
      <div className="grid gap-5 md:grid-cols-2">
        ...
        <ActionHubCard title="AI Command Centre" subtitle="Prepare workforce actions from Copilot." target="Workforce AI Copilot" setActive={setActive} />
...
function DocumentHubScreen({
  ...
  const canManageMasterTemplates =
    resolveTenantPermissionLayer(userRole, userEmail) === "owner" || isVyronMasterOperator(userRole, userEmail);
...
function HrCasesWithWhatsAppActions({
  ...
  async function updateCaseAndSendWhatsApp(hrCase: HrCaseRow, status: "resolved" | "closed") {
    ...
    try {
      ...
      const response = await authFetch("/api/hr-cases/whatsapp-action", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
...
function resolveNavigationTarget(item: string) {
  const aliases: Record<string, string> = {
    ...
    "Mobile Workforce Hub": "Mobile Workforce Hub",
    "Workforce Journey": "Workforce Journey",
    "Travel Intelligence": "Travel Intelligence",
    "Route History": "Route History",
    "Vehicle Intelligence": "Vehicle Intelligence",
    "Workforce Cost Intelligence": "Workforce Cost Intelligence",
    "Profitability Intelligence": "Profitability Intelligence",
    "Client Portal": "Client Portal",
    "Training Centre": "Training Centre",
    "Pilot Readiness": "Pilot Readiness",
    "Workforce Risk Intelligence": "Workforce Risk Intelligence",
    "AI Command Centre": "Workforce AI Copilot",
    "WhatsApp Command Centre": "WhatsApp Command Centre",
    "Workforce Digital Twin": "Workforce Digital Twin",
    "Payroll Intelligence Dashboard": "Payroll Intelligence Dashboard",
    "Payroll Readiness": "Payroll Readiness",
    "Payroll Leakage": "Payroll Leakage",
    "Payroll Forecast": "Payroll Forecast",
    "Payroll Exceptions": "Payroll Exceptions",
    "Workforce Lifecycle": "Workforce Lifecycle",
    ...
    "Operational Command Centre": "Operational Command Centre",
    "Workforce Operating Dashboard": "Workforce Operating Dashboard",
    "Automation Library": "Automation Library",
    "Workforce Health Score": "Workforce Health Score",
  };
...
export default function Page() {
  ...
  function setActive(next: string) {
    if (
      ...
      vyronDevSupportSession &&
      !isRouteAllowedDuringSupportSession(
        next,
        isVyronCoreSupportSession(vyronDevSupportSession)
      )
    ) {
      next = "Login As Client";
    }
  ...
  const [vyronDevOpenProductCode, setVyronDevOpenProductCode] = useState<VyronProductCode | null>(null);
  const {
    platformState: vyronDevPlatformState,
    activeClient: vyronDevActiveClient,
    supportSession: vyronDevSupportSession,
    persistenceStatus: vyronDevPersistenceStatus,
    packagesSynced: vyronDevPackagesSynced,
  ...
  const handleDeleteClientDirectoryEntry = React.useCallback(
    async (entry: MasterClientDirectoryEntry) => {
      ...
      const confirmed = window.confirm(
        `Archive workspace "${entry.companyName}"?\n\nThe client is soft-archived (not hard-deleted). Restore from Company Setup if needed.`
      );
      if (!confirmed) return;
      ...
      void writeAuditLog(supabase, {
        companyId: entry.id,
        userEmail: normalizedAuthEmail || VYRON_MASTER_OPERATOR_EMAIL,
        action: "archive",
        entityType: "client",
        entityId: entry.id,
    ...
    [normalizedAuthEmail, patchClientDirectoryEntry]
  );
  ...
  const handleSuspendDevClient = React.useCallback(
    async (entry: MasterClientDirectoryEntry) => {
      const existing = vyronDevPlatformState.clientProfiles[entry.id];
      persistPlatformState(
        upsertClientProfile(vyronDevPlatformState, {
          clientId: entry.id,
          tradingName: existing?.tradingName || entry.companyName,
          industry: existing?.industry || "General",
          status: "suspended",
          activeUserCount: existing?.activeUserCount ?? 1,
          updatedAt: new Date().toISOString(),
        })
      );
      if (isDeletableCompanyId(entry.id)) {
        await handleSetClientSubscriptionHold(entry, "on-hold");
      }
    },
    [vyronDevPlatformState, persistPlatformState, handleSetClientSubscriptionHold]
  );

  const handleCloneDemoClient = React.useCallback(
    async (entry: MasterClientDirectoryEntry) => {
      const cloneName = `${entry.companyName} (Demo Clone)`;
      const { company, error: provisionError } = await provisionClientCompany(supabase, cloneName, {
        adminEmail: entry.primaryAdminEmail || undefined,
      });
      if (provisionError || !company?.id) {
        alert(provisionError || "Could not clone demo client.");
        return;
      }

      const demoStartedIso = new Date().toISOString();
      await supabase
        .from("companies")
        .update({
      ...
      const sourceProfile = vyronDevPlatformState.clientProfiles[entry.id];
      const newEntry: MasterClientDirectoryEntry = {
        id: company.id,
        companyName: cloneName,
        primaryAdminEmail: entry.primaryAdminEmail,
        ...
        phone: entry.phone,
        demoStartedAt: demoStartedIso,
      };
      appendClientDirectoryEntry(newEntry);

      let nextState = ensureClientProfile(vyronDevPlatformState, company.id, {
        tradingName: sourceProfile?.tradingName || entry.companyName,
        industry: sourceProfile?.industry || "General",
        status: "trial",
      });
      nextState = setClientProductStatus(nextState, company.id, "CORE", "trial", "pkg-core-starter");
      persistPlatformState(nextState);
    },
    [appendClientDirectoryEntry, persistPlatformState, vyronDevPlatformState]
  );

  const openVyronDevProduct = React.useCallback(
    (clientId: string, productCode: VyronProductCode) => {
      const entry = clientDirectory.find((item) => item.id === clientId);
      if (!entry) return;
      const profile = vyronDevPlatformState.clientProfiles[clientId];
      persistActiveClient({
        clientId,
        companyName: entry.companyName,
        tradingName: profile?.tradingName || entry.companyName,
        primaryContactEmail: entry.primaryAdminEmail,
        selectedAt: new Date().toISOString(),
      });
      persistWorkspaceOpen(clientId, productCode);
      setVyronDevOpenProductCode(productCode);

      if (productCode === "CORE") {
        const existingSession =
          vyronDevSupportSession?.clientId === clientId
            ? vyronDevSupportSession
            : readVyronDevSupportSession()?.clientId === clientId
              ? readVyronDevSupportSession()
              : null;

        if (!existingSession || existingSession.productCode !== "CORE") {
          const { state, session } = startSupportSession(vyronDevPlatformState, {
            operator: normalizedAuthEmail || authUserEmail || "master-operator",
            clientId,
            clientName: entry.companyName,
            productCode: "CORE",
          });
          persistPlatformState(state);
          const supportContext: VyronSupportSessionContext = { ...session, supportMode: true };
          persistSupportSession(supportContext);
        } else if (!existingSession.supportMode) {
          persistSupportSession({ ...existingSession, supportMode: true });
        }

        setActiveSidebarGroup("Command Centre");
        setActive("Command Centre");
        setRefreshKey((value) => value + 1);
        return;
      }

      setActive("Product Workspace");
    },
    [
      clientDirectory,
  ...
  const handleEndVyronDevSupportSession = React.useCallback(() => {
    if (!vyronDevSupportSession) return;
    void writeAuditLog(supabase, {
      companyId: vyronDevSupportSession.clientId,
      userEmail: normalizedAuthEmail || vyronDevSupportSession.operator,
      action: "exit_client_mode",
      entityType: "support_session",
      entityId: vyronDevSupportSession.sessionId,
      metadata: {
        clientName: vyronDevSupportSession.companyName,
        productCode: vyronDevSupportSession.productCode,
      },
    });
    endClientSupportSession(vyronDevSupportSession.sessionId);
    persistActiveClient(null);
    setVyronDevOpenProductCode(null);
    setCurrentCompanyId("");
    setCurrentCompanyName("VYRON Platform");
    setActiveSidebarGroup("PLATFORM CONTROL");
    refreshData();
    setActive("Client Directory");
  }, [
    vyronDevSupportSession,
    endClientSupportSession,
    normalizedAuthEmail,
  ...
  const handleReturnToVyronDevFromSupport = React.useCallback(() => {
    setActive("Client Directory");
  }, [setActive]);

  const handleResolveProvisionCompanyId = React.useCallback(
    async (input: {
      companyName: string;
      email: string;
      password: string;
      ...
      if (provisionError || !company?.id) return null;
      ...
      const tier =
        input.status === "trial"
          ...
          : input.status === "suspended"
            ? "Starter"
            : "Starter";
      const fee = VYRON_SUBSCRIPTION_TIER_MONTHLY_FEES[tier as (typeof CLIENT_SUBSCRIPTION_TIERS)[number]] ?? 1_499;
      ...
      if (!accessToken) {
        return {
          companyId: company.id,
          loginWarning:
            "Client was created, but login user could not be created because operator session token was missing.",
        };
      }
      ...
      if (!loginResult.ok) {
        return { companyId: company.id, loginWarning: loginResult.message };
      }

      return { companyId: company.id, loginWarning: null };
    },
    []
  );

  const handleVyronDevProvisionClient = React.useCallback(
    async (payload: {
      clientId: string;
      companyName: string;
      tradingName: string;
      ...
      appendClientDirectoryEntry({
        id: payload.clientId,
        companyName: payload.companyName,
        primaryAdminEmail: payload.email,
        ...
        subscriptionStatus:
          payload.status === "trial"
            ...
            : payload.status === "suspended"
              ...
              : "active",
        inviteStatus: "Pending Setup",
        registrationDate: payload.createdAt.slice(0, 10),
        contactPerson: payload.primaryContact,
        phone: payload.phone,
      });
    },
    [appendClientDirectoryEntry]
  ...
  useEffect(() => {
    ...
    async function loadMasterClientDirectory() {
      ...
      const directory = await queryCompaniesDirectory();
      const companyRows = directory.error ? undefined : directory.rows;
  ...
  useEffect(() => {
    if (!isMasterOperatorSession || clientDirectory.length === 0) return;
    let next = readVyronDevPlatformState();
    let changed = false;
    for (const entry of clientDirectory) {
      if ((entry.companyStatus || "").toLowerCase() === "archived") continue;
      const beforeLen = next.productWorkspaces.length;
      next = ensureClientProfile(next, entry.id, {
        tradingName: entry.companyName,
        industry: "General",
      });
      const hasCore = next.productWorkspaces.some(
        (ws) => ws.clientId === entry.id && ws.productCode === "CORE"
      );
      if (!hasCore) {
        next = setClientProductStatus(next, entry.id, "CORE", "enabled", "pkg-core-starter");
        changed = true;
      } else if (next.productWorkspaces.length !== beforeLen) {
        changed = true;
      }
    }
    if (changed) {
      persistPlatformState(next);
    }
  }, [clientDirectory, isMasterOperatorSession, persistPlatformState]);
  ...
  useEffect(() => {
    async function loadData() {
      ...
      if (isMasterOperator) {
        await getPlatformWorkspaceId(supabase);
      }
      const { access, error: accessError } = await getCompanyAccess(supabase);

      const resolvedAccess = isMasterOperator ? getMasterOperatorCompanyAccess() : access;
      ...
      if (!isMasterOperator && (resolvedAccessError || !resolvedAccess?.company_id)) {
        if (
          resolvedAccessError &&
          !shouldSuppressWorkspaceLoadMessage(resolvedAccessError)
        ) {
      ...
      const storedSupportSession =
        typeof window !== "undefined" ? readVyronDevSupportSession() : null;
      const activeSupportSession = isVyronCoreSupportSession(vyronDevSupportSession)
        ? vyronDevSupportSession
        : isVyronCoreSupportSession(storedSupportSession)
          ? storedSupportSession
          : null;
      const supportCoreSession =
        isMasterOperator && activeSupportSession ? activeSupportSession : null;

      if (isMasterOperator && !resolvedAccess?.company_id && !supportCoreSession) {
        const platformAccess = resolvedAccess ?? getMasterOperatorCompanyAccess();
        setCurrentCompanyId("");
        syncLayoutRole(platformAccess.user_role);
        setCurrentCompanyName(platformAccess.company_name || "VYRON Platform");
        setStores([]);
        setEmployees([]);
        setExceptions([]);
        setHrCases([]);
      ...
      if (!resolvedAccess?.company_id && !supportCoreSession) {
        setLoading(false);
        return;
      }
      ...
      if (
        !supportCoreSession &&
        resolvedAccess &&
        String((resolvedAccess as VyronCompanyAccess & { company_status?: string }).company_status || "active").toLowerCase() !== "active"
      ) {
      ...
      if (!supportCoreSession && !masterOperatorBypass && resolvedAccess && subscriptionStatus === "on-hold") {
      ...
      const activeCompanyId = supportCoreSession
        ? supportCoreSession.clientId
        : resolvedAccess!.company_id;
      const activeCompanyName = supportCoreSession
        ? supportCoreSession.companyName
        : resolvedAccess!.company_name || "Active company";

      setCurrentCompanyId(activeCompanyId);
      syncLayoutRole(
        supportCoreSession ? VYRON_MASTER_OPERATOR_ROLE : resolvedAccess?.user_role
      );
      ...
      const [storesRes, employeesRes, exceptionsRes, rosterRes, shiftTemplatesRes, rosterRulesRes, coverageRequirementsRes, clockRes, payrollRes, payrollHoursRes, payrollClockChecksRes, rolesRes, companyUsersRes, hrWarningsRes, hrDocumentsRes, hrNotesRes, leaveRequestsRes, companyPlanRes] = await Promise.all([
        ...
        supabase
          .from("employees")
          .select("id,employee_number,first_name,last_name,job_title,default_store_id,active,email,phone,employment_type,pin_code,kiosk_access_enabled,record_status")
          .eq("company_id", activeCompanyId)
          .order("first_name"),
      ...
      const optionalTableMissing = (table: string, err: { code?: string; message?: string } | null) =>
        isSupabaseMissingTableError(err, table) || shouldSuppressWorkspaceLoadError(err);

      const employeeDocumentsTableMissing = optionalTableMissing(
        "employee_documents",
        employeeDocumentsRes.error
      );
      const payrollClockChecksTableMissing = optionalTableMissing(
      ...
      const hrWarningsTableMissing = optionalTableMissing("hr_warnings", hrWarningsRes.error);
      const hrDocumentsTableMissing = optionalTableMissing("hr_documents", hrDocumentsRes.error);
      const hrNotesTableMissing = optionalTableMissing("hr_notes", hrNotesRes.error);
      const leaveRequestsTableMissing = optionalTableMissing(
        "leave_requests",
      ...
      const firstError =
        ...
        (payrollClockChecksTableMissing ? null : payrollClockChecksRes.error) ||
        rolesRes.error ||
        (companyUsersTableMissing ? null : companyUsersRes.error) ||
        (employeeDocumentsTableMissing ? null : employeeDocumentsRes.error) ||
        (hrWarningsTableMissing ? null : hrWarningsRes.error) ||
        (hrDocumentsTableMissing ? null : hrDocumentsRes.error) ||
        (hrNotesTableMissing ? null : hrNotesRes.error) ||
        (leaveRequestsTableMissing ? null : leaveRequestsRes.error);

      if (firstError && !shouldSuppressWorkspaceLoadError(firstError)) {
        setError(firstError.message);
      }

      const liveStores = (storesRes.error ? [] : storesRes.data || []).filter(
        (row) => String((row as { record_status?: string }).record_status || "active") !== "deleted"
      );
      const liveEmployees = (employeesRes.error ? [] : employeesRes.data || []).filter(
        (row) => String((row as { record_status?: string }).record_status || "active") !== "deleted"
      );
      setStores(liveStores as StoreRow[]);
      setEmployees(liveEmployees as EmployeeRow[]);
      ...
      setPayrollClockChecks(
        (payrollClockChecksTableMissing || payrollClockChecksRes.error
          ? []
          : payrollClockChecksRes.data || []) as PayrollClockCheckRow[]
      );
      setUserRoles((rolesRes.error ? [] : rolesRes.data || []) as UserRoleRow[]);
      setCompanyUsers(
        (companyUsersTableMissing || companyUsersRes.error
          ? []
          : companyUsersRes.data || []) as CompanyUserRow[]
      );
      setHrWarnings((hrWarningsRes.error ? [] : hrWarningsRes.data || []) as HrWarningRow[]);
      setHrDocuments((hrDocumentsRes.error ? [] : hrDocumentsRes.data || []) as HrDocumentRow[]);
      setEmployeeDocuments(
        (employeeDocumentsTableMissing || employeeDocumentsRes.error
          ? []
          : employeeDocumentsRes.data || []) as EmployeeDocumentRow[]
      );
      setHrNotes((hrNotesRes.error ? [] : hrNotesRes.data || []) as HrNoteRow[]);
      setLeaveRequests(
        (leaveRequestsRes.error ? [] : leaveRequestsRes.data || []) as LeaveRequestRow[]
      );
      ...
      if (cleanEmail && activeCompanyId) {
        void registerUserSession(supabase, {
          userEmail: cleanEmail,
          companyId: activeCompanyId,
          userAgent: typeof navigator !== "undefined" ? navigator.userAgent : null,
        });
        const sessionToken = readLocalSessionToken(cleanEmail);
        if (sessionToken) {
          void touchUserSession(supabase, sessionToken);
        }
      }
    ...
    loadData();
  }, [today, refreshKey, authUserEmail, vyronDevSupportSession?.clientId, vyronDevSupportSession?.productCode]);
  ...
  function renderSection() {
    ...
    if (active === "WhatsApp Command Centre")
      return (
        <WhatsAppCommandCentrePanel
    ...
    if (active === "Command Centre")
      return isMasterOperatorSession && !isVyronCoreSupportView ? (
    ...
    if (active === "Manager Action Centre") return <ManagerActionCentrePanel onNavigate={setActive} companyId={currentCompanyId} />;
    ...
    if (active === "Automation Centre")
      return (
        <AutomationWorkflowHubScreen
          setActive={setActive}
          companyId={currentCompanyId}
          userEmail={normalizedAuthEmail}
        />
    ...
    if (active === "Job Visits")
      return (
        <JobVisitsPanel companyId={currentCompanyId} employees={employees} stores={stores} />
      );
    if (active === "Job Timeline")
      return <JobTimelineEngine companyId={currentCompanyId} employees={employees} />;
    if (active === "Field Mobile" || active === "Mobile Workforce Hub")
      return (
        <MobileWorkforceHub
          companyId={currentCompanyId}
          employees={employees}
          ...
          userEmail={normalizedAuthEmail}
          canAssignTasks={
            tenantPermissionLayer === "owner" ||
            tenantPermissionLayer === "admin" ||
            tenantPermissionLayer === "manager" ||
            tenantPermissionLayer === "supervisor"
          }
        />
      );
    if (active === "Workforce Journey")
      return <WorkforceJourneyPanel companyId={currentCompanyId} employees={employees} />;
    if (active === "Travel Intelligence")
      return <TravelIntelligencePanel companyId={currentCompanyId} employees={employees} />;
    if (active === "Route History")
      return <RouteHistoryPanel companyId={currentCompanyId} employees={employees} />;
    if (active === "Vehicle Intelligence")
      return (
        <VehicleAssetIntelligencePanel companyId={currentCompanyId} employees={employees} />
      );
    if (active === "Workforce Cost Intelligence")
      return (
        <WorkforceCostIntelligencePanel companyId={currentCompanyId} employees={employees} />
      );
    if (active === "Profitability Intelligence")
      return (
        <ClientProfitabilityIntelligencePanel
          companyId={currentCompanyId}
          employees={employees}
        />
      );
    if (active === "Client Portal")
      return (
        <ClientPortalAdminPanel companyId={currentCompanyId} employees={employees} />
      );
    if (active === "Training Centre") return <TrainingCentrePanel companyId={currentCompanyId} />;
    if (active === "Pilot Readiness")
      return (
        <PilotClientReadinessProgram
          companyId={currentCompanyId}
          companyName={currentCompanyName}
          companyContactSet={clientDirectory.some(
            (entry) =>
              entry.id === currentCompanyId && Boolean(String(entry.contactPerson || "").trim())
          )}
          stores={stores}
          employees={employees}
          rosterShifts={rosterShifts}
    ...
    if (active === "Workforce Risk Intelligence")
      return <WorkforceRiskIntelligencePanel companyId={currentCompanyId} />;
    if (active === "Workforce AI Copilot")
      return (
        <WorkforceAiCopilotPanel
          companyId={currentCompanyId}
          userEmail={normalizedAuthEmail}
          onOpenAutomation={() => setActive("Automation Centre")}
        />
      );
    if (active === "Workforce Digital Twin")
      return (
        <WorkforceDigitalTwinPanel
          companyId={currentCompanyId}
          userEmail={normalizedAuthEmail}
        />
      );
    if (active === "Payroll Intelligence Dashboard")
      return <PayrollIntelligencePanel companyId={currentCompanyId} initialView="dashboard" />;
    if (active === "Payroll Readiness")
      return <PayrollIntelligencePanel companyId={currentCompanyId} initialView="readiness" />;
    if (active === "Payroll Leakage")
      return <PayrollIntelligencePanel companyId={currentCompanyId} initialView="leakage" />;
    if (active === "Payroll Forecast")
      return <PayrollIntelligencePanel companyId={currentCompanyId} initialView="forecast" />;
    if (active === "Payroll Exceptions")
      return <PayrollIntelligencePanel companyId={currentCompanyId} initialView="exceptions" />;
    if (active === "Workforce Lifecycle")
      return <WorkforceLifecyclePanel companyId={currentCompanyId} />;
    if (active === "Recruitment Intelligence")
      return <RecruitmentIntelligencePanel companyId={currentCompanyId} initialView="dashboard" />;
    if (active === "Vacancies")
      return <RecruitmentIntelligencePanel companyId={currentCompanyId} initialView="vacancies" />;
    if (active === "Applicants")
      return <RecruitmentIntelligencePanel companyId={currentCompanyId} initialView="applicants" />;
    if (active === "Interviews")
      return <RecruitmentIntelligencePanel companyId={currentCompanyId} initialView="interviews" />;
    if (active === "Skills Intelligence")
      return <RecruitmentIntelligencePanel companyId={currentCompanyId} initialView="skills" />;
    if (active === "Succession Planning")
      return <RecruitmentIntelligencePanel companyId={currentCompanyId} initialView="succession" />;
    if (active === "Executive Command Centre")
      return <WorkforceOperatingSystemPanel companyId={currentCompanyId} initialView="executive" />;
    if (active === "Operational Command Centre")
      return <WorkforceOperatingSystemPanel companyId={currentCompanyId} initialView="operational" />;
    if (active === "Workforce Operating Dashboard")
      return <WorkforceOperatingSystemPanel companyId={currentCompanyId} initialView="dashboard" />;
    if (active === "Automation Library")
      return <WorkforceOperatingSystemPanel companyId={currentCompanyId} initialView="automation" />;
    ...
    if (active === "Leave Balance Control") return <LeaveBalancePanel companyId={currentCompanyId} onUpdated={refreshData} />;
    if (active === "Leave Decision Audit") return <LeaveDecisionAuditPanel />;

    if (active === "HR Cases") return <HrCasesWithWhatsAppActions hrCases={hrCases} employees={employees} companyId={currentCompanyId} onRefresh={refreshData} />;
    ...
    if (active === "Mobile Workforce")
      return (
        <MobileWorkforceScreen
          companyId={currentCompanyId}
          employees={employees}
          stores={stores}
          userEmail={normalizedAuthEmail}
    ...
    if (active === "Client Directory")
      return isMasterOperatorSession ? (
        <VyronDevClientDirectoryScreen
          entries={clientDirectory}
          platformState={vyronDevPlatformState}
          onPlatformStateChange={persistPlatformState}
          onCreateClient={() => setActive("Client Setup")}
          onEditClient={(entry) => setClientDirectoryDetailEntry(entry as MasterClientDirectoryEntry)}
          onSuspendClient={(entry) =>
            handleSuspendDevClient(entry as MasterClientDirectoryEntry)
          }
          onDeleteClient={(entry) =>
            handleDeleteClientDirectoryEntry(entry as MasterClientDirectoryEntry)
          }
          onCloneDemoClient={(entry) =>
            handleCloneDemoClient(entry as MasterClientDirectoryEntry)
          }
          onOpenClientDetail={(entry) => setClientDirectoryDetailEntry(entry as MasterClientDirectoryEntry)}
          onOpenProduct={openVyronDevProduct}
        />
      ) : (
        <ClientDirectoryScreen
          clientDirectory={clientDirectory}
          onArchiveClient={undefined}
          onDeleteClient={undefined}
          onSetSubscriptionHold={undefined}
          onMasterChangeTier={undefined}
          onOpenClientDetail={undefined}
          onResendInvite={undefined}
        />
      );
    if (active === "Login As Client" && isMasterOperatorSession)
      return (
        <VyronDevActiveClientPanel
          entries={clientDirectory}
          platformState={vyronDevPlatformState}
          operatorEmail={normalizedAuthEmail || authUserEmail || "master-operator"}
          supportSession={vyronDevSupportSession}
          onPlatformStateChange={persistPlatformState}
          onStartSupportSession={(session) => {
            persistSupportSession({
              ...session,
              supportMode: session.productCode === "CORE" ? true : session.supportMode,
            });
            const entry = clientDirectory.find((item) => item.id === session.clientId);
            if (entry) {
              const profile = vyronDevPlatformState.clientProfiles[session.clientId];
              persistActiveClient({
                clientId: session.clientId,
                companyName: entry.companyName,
                tradingName: profile?.tradingName || entry.companyName,
                primaryContactEmail: entry.primaryAdminEmail,
                selectedAt: session.startedAt,
              });
            }
          }}
          onEndSupportSession={handleEndVyronDevSupportSession}
          onOpenProduct={(code) => {
            const clientId =
              vyronDevSupportSession?.clientId || vyronDevActiveClient?.clientId;
            if (!clientId) return;
            openVyronDevProduct(clientId, code);
          }}
        />
      );
    if (active === "Client Provisioning" && isMasterOperatorSession)
      return (
        <VyronDevProvisioningPanel
          entries={clientDirectory}
          platformState={vyronDevPlatformState}
          onPlatformStateChange={persistPlatformState}
          onResolveCompanyId={handleResolveProvisionCompanyId}
          onProvisionClient={handleVyronDevProvisionClient}
          persistenceStatus={vyronDevPersistenceStatus}
          onSyncProvisionedClient={syncProvisionedClient}
        />
      );
    if (active === "Workspace Provisioning" && isMasterOperatorSession)
      return (
        <VyronDevWorkspaceProvisioningPanel
          entries={clientDirectory}
          platformState={vyronDevPlatformState}
          onPlatformStateChange={persistPlatformState}
          onOpenWorkspace={openVyronDevProduct}
        />
      );
    if (active === "Package Engine" && isMasterOperatorSession)
      return (
        <VyronDevPackageEnginePanel
          platformState={vyronDevPlatformState}
          packagesSynced={vyronDevPackagesSynced}
          persistenceStatus={vyronDevPersistenceStatus}
        />
      );
    if (active === "Billing" && isMasterOperatorSession)
      return (
        <VyronDevBillingPanel entries={clientDirectory} platformState={vyronDevPlatformState} />
      );
    if (active === "Deployment Centre" && isMasterOperatorSession)
      return <VyronDevDeploymentCentrePanel />;
    if (active === "Integrations Status" && isMasterOperatorSession)
      return (
        <VyronDevIntegrationsPanel
          entries={clientDirectory}
          platformState={vyronDevPlatformState}
          onPlatformStateChange={persistPlatformState}
        />
      );
    if (active === "Product Workspace" && isMasterOperatorSession)
      return (
        <VyronDevProductWorkspaceScreen
          productCode={vyronDevOpenProductCode || vyronDevSupportSession?.productCode || "CORE"}
          activeClient={vyronDevActiveClient}
          supportSession={vyronDevSupportSession}
          platformState={vyronDevPlatformState}
          onBack={goBack}
          onOpenCore={() => setActive("Command Centre")}
          onWorkspaceOpened={persistWorkspaceOpen}
        />
      );
    ...
    if (active === "Payroll Hardening") return <PayrollHardeningScreen companyId={currentCompanyId} />;
    ...
    if (active === "History Reports") return <HistoryReportsPanel companyId={currentCompanyId} />;
