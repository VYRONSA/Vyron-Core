...
export default function MobileWorkforcePage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (!activeCompanyId) {
        setError("Sign in as a tenant user or use VYRON DEV to access mobile workforce.");
        setLoading(false);
        return;
      }
  ...
  return (
    <main className="min-h-screen bg-[#f6f8fb] p-4 text-slate-950">
      <div className="mx-auto max-w-lg space-y-4">
        ...
        {loading ? (
          <p className="text-sm font-semibold text-slate-500">Loading mobile workforce hub…</p>
        ) : error ? (
