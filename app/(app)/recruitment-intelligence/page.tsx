  ...
  return (
    <main className="min-h-screen bg-[#f6f8fb] p-4 text-slate-950 md:p-8">
      <section className="mx-auto max-w-6xl space-y-6">
        ...
        {loading ? (
          <p className="text-sm font-semibold text-slate-500">Loading Recruitment Intelligence…</p>
        ) : error ? (
          <p className="rounded-2xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm font-semibold text-rose-800">{error}</p>
        ) : (
          <RecruitmentIntelligencePanel companyId={companyId} initialView="dashboard" />
        )}
      </section>
    </main>
  );
