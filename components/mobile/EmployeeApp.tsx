...
export default function EmployeeApp() {
  ...
  const openLink = useCallback((raw: string) => {
    ...
    else if (link.includes("/job") || link.includes("/work")) setTab("work");
      ...
      <main className="min-h-0 flex-1 overflow-y-auto px-4 pb-28 pt-4">
        {reporting ? (
          ...
          <>
            ...
            {tab === "inbox" && <InboxTab notifications={notifications.data?.notifications || []} />}
            {tab === "more" && <MoreTab />}
          </>
        )}
      </main>
...
function InboxTab({ notifications }: { notifications: NotificationRow[] }) {
      ...
      {notifications.map((note) => {
        ...
        return (
          <div
            ...
            className={`rounded-[22px] px-4 py-3 ${unread ? "bg-cyan-50" : "bg-white"}`}
          >
